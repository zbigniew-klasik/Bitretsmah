...
namespace Bitretsmah.Tests.Unit.Core
{
    public class ChangedFilesUploaderShould
    {
        ...
        public void Setup()
        {
            ...
            _changedFilesUploader = new ChangedFilesUploader(_fileHashServiceMock.Object, _localFilesServiceMock.Object,
                _loggerMock.Object, _remoteFileWarehouseFactoryMock.Object);
        }
        ...
        public async Task ComputeHashForEachFileWithEmptyHash()
        {
            ...
            _fileHashServiceMock.Verify(x => x.TryEnsureFileHasComputedHash(It.IsAny<File>(), progress),
                Times.Exactly(2));
        ...
        public async Task UploadsEachFileNotUploadedYet()
        {
            ...
            _localFilesServiceMock.Setup(x => x.ReadFileStream(@"C:\Temp\file_not_yet_uploaded.txt"))
                .Returns(fileStream);
            ...
            var fileUploadedBefore = CreateFile("file_uploaded_before.txt", NodeState.Modified,
                "A8499F0A32485CB682D5AB7050F967CFBA504D84");
            var fileNotYetUploaded = CreateFile("file_not_yet_uploaded.txt", NodeState.Modified,
                "F7031B995F0F2E9CED6D62156F3DEB756ADB7E1E");
            var filesStructureChange = CreateDirectory("root", NodeState.Modified, fileUploadedBefore,
                fileNotYetUploaded);

            await _changedFilesUploader.Upload(filesStructureChange, new Progress<BackupProgress>());

            _remoteFileWarehouseMock.Verify(
                x => x.UploadFile(It.IsAny<Stream>(), It.IsAny<string>(), It.IsAny<Progress<double>>()), Times.Once);
            _remoteFileWarehouseMock.Verify(
                x =>
                    x.UploadFile(fileStream, "[F7031B995F0F2E9CED6D62156F3DEB756ADB7E1E]_file_not_yet_uploaded.txt",
                        It.IsAny<Progress<double>>()), Times.Once);
        ...
        public async Task ComputeMissingHashAndUseItInUploadedFileName()
        {
            ...
            _fileHashServiceMock.Setup(
                    x => x.TryEnsureFileHasComputedHash(fileWithEmptyHash, new Progress<BackupProgress>()))
